...
- 2026-09-25 (r006): Operator rule — main-page buttons never exceed sidebar nav button dimensions (28×130). Caps live in config_layout MAIN_BUTTON_*; enforced by tests/test_button_caps.py which builds the real window and measures preferred sizes of every content-area button. Button icons are 14px. GTK3 note: the starter's `* { font-size: 11pt }` matches child labels directly and overrides parent-class font inheritance — button labels need explicit `.class label {}` rules. Also fixed: style.css was never loaded at startup (only ThemeApplicator's inline colors); main.py now calls ThemeManager().load_css.
